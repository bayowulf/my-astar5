...
func fire():
	print_rich("[font_size=15][color=Deepskyblue] Turrets.gd fire()")
	readyfreddy = false
	
	if category == "Projectile":
		fire_gun()
		#print_rich("[font_size=15][color=cyan]enemy = ", enemy, "; enemy.name = ", enemy.name)
		#print_rich("[font_size=15][color=cyan]enemy get has method on_hit = ", enemy.has_method("on_hit"))
		...
		## NOTE this is the spot in the code to reload. creaate a timer cooldown and fire missile again
		## missile_ready is a flag that is useful if I wanted to animate a reload sequence
		##currently missile_ready is always true
		#if missile_ready: 
			#fire_missile()
			fire_missile()
			await(get_tree().create_timer(GameData.tower_data[type]["rof"]).timeout)
...
func fire_gun():
	print_rich("[font_size=15][color=lawngreen] fire_gun pow pow pow")
...
##2.point towards enemy 3. update current direction to desired direction 4.repeat until missile enters the collisionshape2d of the enemy tank
	print_rich("[font_size=15][color=wheat]Turrets.gd func fire_missile()")
	##NOTE missile_ready = false  ## useful if I wanted to animate a reload sequence
	
	var rocket = rocket_prefab.instantiate()
	
	## SIGNAL 'rocket_impact' emitted by rocket collisionshape2d on body entered.
