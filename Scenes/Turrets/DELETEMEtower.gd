...
### NOTE signal is  tower scene/Area2D -> signal body_shape_entered.
### NOTE the signal is disconnected currently.
#func _on_area_2d_input_event(_viewport: Node, event: InputEvent, _shape_idx: int) -> void:
	##print("event = ", event)
		...
		#var tile = GameData.ground.local_to_map(get_global_mouse_position())
#
